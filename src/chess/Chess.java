...
public class Chess {
	...
	static class BlockedException extends Exception {
		public BlockedException() {}
	}
	 
	@SuppressWarnings("serial")
	static class WrongColorException extends Exception {
	 ...
	 * 
	 */
	private static void movePiece(String move, Board board, Color color) throws Exception {
		Color opposite;
		if (color == Color.WHITE) {
		...
		int[] ranksAndFiles = parseMove(move);
	 ...
	 * 
	 */
	private static void promotePawn(String move, Board board, Color color) throws Exception {
		if(move.charAt(5) != ' '){
			throw new IllegalArgumentException();
		}
		String promoteTo = ("" + move.charAt(6)).toLowerCase(); //character representation of the piece to promote to
		int[] ranksAndFiles = parseMove(move.substring(0, 5));
	 ...
	 */
	private static void updateProtectedSquares(Board b) {
	 ...
	 */
	private static void inCheck(Board b, Color c) {
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
		...
		if (p.getPieceType() != PieceType.PAWN) {
			...
			if (p.getColor() == Color.WHITE) {
				...
				if (black_moves.isEmpty()) {
					...
					int[] black_last_move = parseMove(black_moves.get(black_moves.size() - 1)); //retrieve last move black made
			...
			//black's move
				...
				if (white_moves.isEmpty()) {
					...
					int[] white_last_move = parseMove(white_moves.get(white_moves.size() - 1)); //retrieve last move white made
	...
	public static void main(String[] args) {
		...
		Color color;
		for (int count = 0; !end && !board.draw() && lastPawnMoveOrCapture < 50; count++) {
			if (count % 2 == 0) {
				System.out.print("\nWhite's move: ");
				color = Color.WHITE;
			}
